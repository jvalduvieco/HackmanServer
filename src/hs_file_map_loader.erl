...
%% API
%% Map functions
-export([load/1, get_map_property/2]).
%% Layer functions
...
get_object_name(Object) ->
	proplists:get_value(<<"name">>, Object, undefined).

translate_coords_to_tile({X,Y}, CellWidth, CellHeight) ->
	{math_tools:floor(X/CellWidth), math_tools:floor(Y/CellHeight)}.
...
load_object_layer(Map, ObjectLayer, ObjectType, Id, TileWidth, TileHeight) ->
	...
	ObjectCoords = translate_coords_to_tile(get_object_position(Object), TileWidth, TileHeight),
	hs_map_store:add_entity(Map, CompletebinId, ObjectType, ObjectCoords, false),
